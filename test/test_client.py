...
class TestTfClient(unittest.TestCase):
    ...
    def test_transform_point(self):
        ...
        tf_client.waitForTransform(target_frame, ps.header.frame_id, rospy.Time(), rospy.Duration(2.0))
        new_pose = tf_client.transformPoint(target_frame, ps, timeout=rospy.Time(0.5))

        self.assertNotEqual(new_pose.point.z, ps.point.z)  # These two frames are not at the same Z
